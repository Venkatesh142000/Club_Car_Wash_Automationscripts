...
> 1. **Code Generation** — Standards and patterns for writing accessible code
> 2. **Auditor Pattern Library** — Reference for `accessibility-auditor.md` when generating fix suggestions
>
> _For automated accessibility testing and reporting, see `accessibility-auditor.md`._
...
## Template and Component Architecture Considerations

### Understanding Template Types

Before applying accessibility requirements, identify what type of template or component you're working with:

#### 1. Full Page Templates

Complete HTML pages with `<!DOCTYPE html>`, `<html>`, `<head>`, and `<body>` tags.

- **Must include**: Page title, skip links, `<main>` landmark, overall page structure
- Examples: Standalone HTML pages, email templates, static HTML files

#### 2. Layout/Wrapper Templates

Wrapper templates that provide page structure for content components.

- **Must include**: Page title (`<title>`), skip links, `<main>` landmark, `<header>`, `<footer>`, overall landmark structure
- **Common indicators**:
  - Templates named: `layout`, `page`, `base`, `master`, `wrapper`
  - Template inheritance/extension syntax: `extends`, `layout`, decorator patterns
  - Contains structural HTML tags: `<!DOCTYPE>`, `<html>`, `<head>`, `<body>`
- Examples: Rails layouts, Django base templates, Laravel blade layouts, React/Vue layout components, JSP/JSF templates, ISML decorators, Twig base templates

#### 3. Content Components (Partials)

Partial templates inserted into layouts/wrappers. These are the most common type.

- **Do NOT require**: Page title, skip links, `<main>` landmark (these are provided by the layout)
- **Must include**: Component-level accessibility features (headings, labels, ARIA, keyboard support, contrast, etc.)
- **Common indicators**:
  - Inclusion/import syntax: `include`, `partial`, `render`, `import`, `require`
  - Templates that wrap content in layout/decorator calls
  - File naming: `_partial`, `component`, or in `components/` directories
  - Does not contain `<!DOCTYPE>`, `<html>`, `<head>`, or `<body>`
- Examples: React/Vue components, Rails partials, Django includes, Laravel components, JSP includes, template fragments

### Applying Requirements Correctly

**When reviewing or generating code:**

1. **First, determine the template type** by looking for:
   - Presence of `<!DOCTYPE>`, `<html>`, `<head>`, `<body>` tags
   - Layout/extension syntax for the templating system
   - Inclusion/import statements
   - File naming patterns and directory structure
   - Project conventions (check framework documentation)

2. **Apply appropriate requirements**:
   - For **content components**: Focus on semantic HTML, headings hierarchy within the component, form labels, ARIA attributes, keyboard navigation, and visual design (contrast, focus indicators)
   - For **layout templates**: Include page-level landmarks, skip links, page title
   - For **full pages**: Include everything

3. **Avoid false positives**: Do not flag missing `<main>`, skip links, or page title in content components that are clearly inserted into layouts.

### Generic Examples

**Layout Template (provides page structure):**
...
**Content Component (inserted into layout):**

```html
<!-- This is a CONTENT COMPONENT -->
<!-- Page title, main, skip links are in the layout -->

<h1>Shopping Cart</h1>
<div class="cart-items">
    <!-- Component content with proper labels, ARIA, etc. -->
</div>
```

**Correct assessment**: The content component needs proper heading hierarchy, form labels, and keyboard support, but does NOT need its own `<main>` or skip links because the layout provides them.
